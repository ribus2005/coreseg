...
class PAttUNet(nn.Module):
    def __init__(self, input_channels=3, num_classes = 1, deep_supervision=False):
        ...
        self.pool1 = DownConv(nb_filter[0], nb_filter[0])
        self.pool2 = DownConv(nb_filter[0], nb_filter[0])
        self.pool3 = DownConv(nb_filter[1], nb_filter[1])
        self.pool4 = DownConv(nb_filter[1], nb_filter[1])
        self.pool5 = DownConv(nb_filter[2], nb_filter[2])
        self.pool6 = DownConv(nb_filter[2], nb_filter[2])
        self.pool7 = DownConv(nb_filter[3], nb_filter[3])
        self.pool8 = DownConv(nb_filter[3], nb_filter[3])

        self.up1 = UpConv(nb_filter[4], nb_filter[4])
        self.up2 = UpConv(nb_filter[3], nb_filter[3])
        self.up3 = UpConv(nb_filter[2], nb_filter[2])
        self.up4 = UpConv(nb_filter[1], nb_filter[1])
        #self.up = nn.Upsample(scale_factor=2, mode='bilinear', align_corners=True)                
    ...
    def forward(self, input):
        ...
        x1_0att = self.Attdw1(g=x11_0, x=self.pool1(x0_0))        
        x1_0 = self.conv1_0(torch.cat((x1_0att, self.pool2(x0_0)),dim=1))

        x12_0 = self.conv12_0(Images[1])
        x22_0 = self.conv22_0(x12_0)         
        x2_0att = self.Attdw2(g=x22_0, x=self.pool3(x1_0))  
        x2_0 = self.conv2_0(torch.cat((x2_0att, self.pool4(x1_0)),dim=1))        
        
        
        x13_0 = self.conv13_0(Images[2])
        x23_0 = self.conv23_0(x13_0)        
        x33_0 = self.conv33_0(x23_0) 
        x3_0att = self.Attdw3(g=x33_0, x=self.pool5(x2_0))        
        x3_0 = self.conv3_0(torch.cat((x3_0att, self.pool6(x2_0)),dim=1))
        
        x14_0 = self.conv14_0(Images[3])  
        x24_0 = self.conv24_0(x14_0)         
        x34_0 = self.conv34_0(x24_0)
        x44_0 = self.conv44_0(x34_0)
        x4_0att = self.Attdw4(g=x44_0, x=self.pool7(x3_0))
        x4_0 = self.conv4_0(torch.cat((x4_0att, self.pool8(x3_0)),dim=1)) 
